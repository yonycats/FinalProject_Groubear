						...
						<table class="table align-middle table-row-dashed fs-6 gy-5">
							...
							<tbody class="text-gray-600 fw-semibold">
								...
								<c:choose>
									...
									<c:otherwise>
										<c:forEach items="${dataList}" var="alarmList">
											<c:if test="${alarmList.alarmYn eq 'N' }">
												<tr>
													<td>${alarmList.alarmCategory }</td>
													<td><a href="${alarmList.alarmUrl }">
															<p>${alarmList.alarmCn}</p>
													</a></td>
													<td><p>${alarmList.alarmCrtDt}</p></td>
													<td><input type="hidden" id="alarmNo" class="alarmNo" value="${alarmList.alarmNo }" />
													<a href="#"
														class="btn btn-icon btn-light-youtube me-5" id="delAlarm"><i
															class="fa-solid fa-xmark fs-4"></i></a></td>
												</tr>
											</c:if>
										</c:forEach>
									</c:otherwise>
